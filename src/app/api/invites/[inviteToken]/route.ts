...
async function inviteFor(token: string) {
  const { data, error } = await adminClient().from("assignments").select("id, active, room:rooms!inner(id, title, status, theme, stage_count)").eq("invite_token", token).maybeSingle();
  const room = Array.isArray(data?.room) ? data.room[0] : data?.room;
  if (error || !data || !data.active || !room || room.status !== "published") return null;
  return { id: data.id as string, room: room as { id: string; title: string; status: string; theme: string; stage_count: number } };
}
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ inviteToken: string }> }) {
  ...
  try {
    ...
    return invite ? NextResponse.json({ assignmentId: invite.id, room: { title: invite.room.title, theme: invite.room.theme, stageCount: invite.room.stage_count } }) : NextResponse.json({ error: "This invite is unavailable." }, { status: 404 });
