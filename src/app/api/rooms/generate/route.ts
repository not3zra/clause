...
async function auditGeneration(
  ...
  try {
    await admin.from("room_generation_audits").insert({
...
export async function POST(request: NextRequest) {
  try {
    if (!sameOrigin(request) || requestTooLarge(request))
    ...
    const rate = await durableRateLimit(
      `generation:${teacherId}`,
      config.perTeacherGenerationHourlyLimit,
      3600,
    );
    if (!rate.allowed)
      return NextResponse.json(
    ...
    try {
      ...
      for (
        ...
        while (true) {
                ...
                messages: [
                  ...
                  },
                  {
                    ...
                    content: JSON.stringify({
                      ...
                      repairInstructions: generationAttempt
                        ? generationRepairInstruction(
                            repairErrors,
                            input.stageCount,
                          )
          ...
          if (response.status === 429 && groqRetries < 3) {
            const message = (data as { error?: { message?: string } } | null)
              ?.error?.message;
            const match =
              typeof message === "string"
                ? message.match(/try again in ([\d.]+)s/i)
                : null;
            const waitSeconds = Math.min(match ? Number(match[1]) : 5, 30);
          ...
          if (!response.ok) {
            ...
            if (response.status === 400 && failedDraft) {
              const result = parseGeneratedRoomDraft(
                failedDraft,
                input.stageCount,
                input.theme,
              );
            ...
            console.error(
              "room_generation_groq_error",
              response.status,
              JSON.stringify(data),
            );
            const providerMessage =
              typeof (data as { error?: { message?: unknown } } | null)?.error
                ?.message === "string"
            ...
            const message =
              response.status === 429
                ...
                : response.status === 413
                  ? "AI room generation request too large. Shorten the topic or instructions and try again."
                  : "AI room generation is temporarily unavailable. Try again shortly.";
            return NextResponse.json(
          ...
          if (result.ok) {
            ...
            return NextResponse.json({
              draft: result.value,
              source: "ai",
              validation: generationAttempt ? "repaired" : "passed",
            });
          }
          repairErrors = result.errors;
          break;
    ...
    console.error("room_generation_error", error);
    return NextResponse.json(
      {
        error: "AI room generation failed. Try again shortly.",
        retryable: true,
      },
      { status: 500 },
    );
