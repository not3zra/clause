...
type AttemptRow = { id: string; current_stage: number; completed_at: string | null; hints_used: number; student_assignment: { student: { full_name: string; roll_number: string } | null } | null };
...
export function TeacherPortal() {
  ...
  const loadResults = useCallback(async () => {
    const [attemptResult, appealResult] = await Promise.all([
      supabase.from("mission_attempts").select("id, current_stage, completed_at, hints_used, student_assignment:student_assignments!inner(student:student_profiles(full_name, roll_number))").order("updated_at", { ascending: false }),
...
function TeacherResults({ appeals, attempts, busy, onRefresh, onResolve }: { appeals: Appeal[]; attempts: AttemptRow[]; busy: boolean; onRefresh: () => Promise<void>; onResolve: (appeal: Appeal, status: "accepted" | "denied" | "overridden", teacherComment: string) => Promise<void> }) {
  ...
  return <div className="panel mt-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">Class overview</p><h2 className="mt-2 text-2xl font-black">Student results</h2></div><button className="secondary-action" disabled={busy} onClick={() => void onRefresh()} type="button">Refresh results</button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="border-b border-[#e8e2d7] text-xs uppercase text-[#657286]"><tr><th className="pb-3">Student</th><th className="pb-3">Progress</th><th className="pb-3">Score</th><th className="pb-3">Hints</th><th className="pb-3">Status</th></tr></thead><tbody>{attempts.map((item) => <tr className="border-b border-[#f7f3eb]" key={item.id}><td className="py-3 font-bold">{item.student_assignment?.student?.full_name ?? "Student"}<span className="ml-2 text-xs font-normal text-[#657286]">{item.student_assignment?.student?.roll_number}</span></td><td className="py-3">{item.current_stage}/3 stages</td><td className="py-3 font-bold">{scoreForProgress(item.current_stage, Boolean(item.completed_at))}%</td><td className="py-3">{item.hints_used}</td><td className="py-3">{item.completed_at ? "Complete" : "In progress"}</td></tr>)}</tbody></table>{!attempts.length && <p className="py-5 text-sm text-[#657286]">No students have started a published room yet.</p>}</div><div className="mt-7 border-t border-[#e8e2d7] pt-5"><p className="eyebrow">Appeals</p><h3 className="mt-2 text-lg font-black">{appeals.filter((item) => item.status === "pending").length} pending review</h3><div className="mt-4 space-y-3">{appeals.map((appeal) => <div className="rounded-md border border-[#e8e2d7] p-4" key={appeal.id}><p className="font-bold">{appeal.mission_attempt?.student_assignment?.student?.full_name ?? "Student"} | {appeal.stage_id}</p><p className="mt-2 text-sm text-[#667085]">{appeal.student_explanation || "No explanation provided."}</p><p className="mt-2 text-xs font-bold uppercase text-[#657286]">{appeal.status}</p>{appeal.status === "pending" && <><label className="mt-3 block text-sm font-bold">Teacher comment<textarea className="input-shell mt-2 min-h-20" onChange={(event) => setComments({ ...comments, [appeal.id]: event.target.value })} value={comments[appeal.id] ?? ""} /></label><div className="mt-3 flex flex-wrap gap-2"><button className="secondary-action" disabled={busy} onClick={() => void onResolve(appeal, "accepted", comments[appeal.id] ?? "")} type="button">Accept</button><button className="secondary-action" disabled={busy} onClick={() => void onResolve(appeal, "denied", comments[appeal.id] ?? "")} type="button">Deny</button><button className="secondary-action" disabled={busy} onClick={() => void onResolve(appeal, "overridden", comments[appeal.id] ?? "")} type="button">Override</button></div></>}{appeal.teacher_comment && <p className="mt-3 text-sm text-[#0f766e]"><strong>Teacher: </strong>{appeal.teacher_comment}</p>}</div>)}</div></div></div>;
}
