...
export function StudentInvite({ inviteToken }: { inviteToken: string }) {
  ...
  return <main className="mx-auto max-w-xl px-5 py-12"><div className="card card-lg"><p className="eyebrow">Room invite</p><h1 className="mt-2 text-3xl font-black">{invite.room.title}</h1><p className="mt-2 text-[#59677a]">Register to join this {invite.room.stageCount}-stage {invite.room.theme} mission. We do not collect your email address.</p><form className="mt-6 space-y-4" onSubmit={register}><label className="block text-sm font-bold">Full name<input className="input mt-2" value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} required /></label><label className="block text-sm font-bold">Roll number<input className="input mt-2" value={form.rollNumber} onChange={(e) => setForm({ ...form, rollNumber: e.target.value })} required /></label><label className="block text-sm font-bold">Username<input className="input mt-2" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} required /></label><label className="block text-sm font-bold">Password<input className="input mt-2" type="password" minLength={8} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required /></label><button className="btn btn-primary w-full" type="submit">Join mission</button></form>{message && <p className="mt-4 text-sm text-[#315b85]" role="status">{message}</p>}</div></main>;
}
...
function AssignedMission({ attempt, invite, onAttempt, onMessage, supabase }: { attempt: Attempt; invite: Invite; onAttempt: (attempt: Attempt) => void; onMessage: (message: string) => void; supabase: ReturnType<typeof createBrowserClient> }) {
  ...
  return <main className={`mission-player theme-${themeClass} mx-auto max-w-3xl px-5 py-10`}><div className="mb-5 flex items-start justify-between gap-4"><div><p className="eyebrow">Your mission</p><h1 className="mt-2 text-3xl font-black">{invite.room.title}</h1><p className="mt-2 text-[#59677a]">{invite.room.theme} | Stage {attempt.current_stage + 1} of {missionStages.length}</p></div><div className="flex flex-wrap justify-end gap-2">{missionStages.map((item, index) => <span className={`token ${index < attempt.current_stage ? "token-ready" : ""}`} key={item.id}>{index < attempt.current_stage ? item.token : "Locked"}</span>)}</div></div><div className="card card-lg"><p className="eyebrow">{stage.title}</p><h2 className="mt-2 text-2xl font-black">{stage.prompt}</h2>
    {stage.id === "surgery" && <><input className="input mt-6" onChange={(event) => setAnswer(event.target.value)} value={answer} /><button className="btn btn-ghost mt-2" onClick={() => setAnswer(missionStages[0].prompt)} type="button">Reset to original</button></>}
    {stage.id === "sort" && <div className="mt-6 grid gap-3">{evidenceCards.map((card) => <div className="card-sm" key={card.sentence}><p className="font-semibold">{card.sentence}</p><div className="mt-3 flex gap-2">{["Agrees", "Needs revision"].map((value) => <button className={`choice ${sorts[card.sentence] === value ? "active" : ""}`} key={value} onClick={() => setSorts({ ...sorts, [card.sentence]: value })} type="button">{value}</button>)}</div></div>)}</div>}
    {stage.id === "rewrite" && <div className="mt-6 space-y-3">{rewrite.map((value, index) => <input className="input" key={index} onChange={(event) => setRewrite(rewrite.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} value={value} />)}</div>}
    <div className="mt-6 flex flex-wrap gap-3"><button className="btn btn-primary" disabled={revealed} onClick={submit} type="button">Check answer</button>{!revealed && <button className="btn btn-ghost" onClick={() => setAppealOpen(!appealOpen)} type="button">Challenge this result</button>}</div>
    {verdict !== "idle" && <div className={`feedback feedback-${verdict === "correct" ? "correct" : "revise"}`}><p className="text-sm"><strong>Checking:</strong> {aiFeedback?.ruleCheck ?? stage.rule}</p><p className="mt-3 font-black">{verdict === "correct" ? aiFeedback?.feedback ?? "Correct: clue token recovered." : revealed ? "Answer revealed after three attempts." : aiFeedback?.feedback ?? "Needs revision: identify the subject before changing the verb."}</p>{aiFeedback && !revealed && <p className="mt-3 text-sm">Hint: {aiFeedback.hint}{aiFeedback.provisionalCredit ? " Provisional credit is recorded for teacher review." : ""}</p>}{revealed && <div className="mt-4 rounded-md bg-white/70 p-3 text-sm leading-6"><strong>Correct answer: </strong>{stageGuidance[stage.id].answer}<br /><strong>Why: </strong>{stageGuidance[stage.id].reasoning}<button className="btn btn-secondary mt-3" onClick={continueWithGuidance} type="button">Continue with guidance</button></div>}</div>}
    ...
    {!revealed && appealOpen && <div className="mt-5 border-t border-[#d8dee8] pt-5"><label className="block text-sm font-bold">Optional explanation<textarea className="input mt-2 min-h-24" onChange={(event) => setExplanation(event.target.value)} value={explanation} /></label><button className="btn btn-secondary mt-3" onClick={submitAppeal} type="button">Submit challenge</button></div>}
  </div></main>;
}
